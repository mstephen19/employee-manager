...
module.exports = {
  view: async function(response) {
    ...
    switch(toShow){
      ...
      case "employees":
        ...
        info = await query('SELECT employees.id, employees.first_name, employees.last_name, employees.role_id AS title, roles.department_id, roles.salary, employees.manager_id FROM employees JOIN roles WHERE employees.role_id = roles.id ORDER BY employees.id');
        break;
    }
    return info;
  },
  ...
  update: async function(data) {
    const res = await query('SELECT * FROM roles WHERE role = ?', data.new_role)
      .then(roleInfo => query('UPDATE employees SET role_id = ? WHERE first_name = ? AND last_name = ?', [roleInfo[0].id, data.employee_name.split(' ')[0], data.employee_name.split(' ')[1]]))
    return res;
  },
  employeeNameArray: async function(){
    const employeeArr = [];
    const employees = await query('SELECT first_name, last_name FROM employees')
